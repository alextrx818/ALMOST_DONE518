#!/bin/bash
# Validate logging configuration and standardization
...
GETLOGGER_COUNT=$(grep -RIn "logging\.getLogger" --include="*.py" --exclude="log_config.py" --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | wc -l)
if [ $GETLOGGER_COUNT -eq 0 ]; then
  ...
  grep -RIn "logging\.getLogger" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | head -5
fi
echo

# Check 2: Every logger comes from factory functions
echo "📋 Check 2: Factory functions usage"
MODULE_COUNT=$(find . -name "*.py" -type f -not -path "./log_config.py" -not -path "./tests/*" -not -path "./tools/*" -not -path "./sports_venv/*" | wc -l)
...
ADDHANDLER_COUNT=$(grep -RIn "\.addHandler" --include="*.py" --exclude="log_config.py" --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | wc -l)
if [ $ADDHANDLER_COUNT -eq 0 ]; then
  ...
  grep -RIn "\.addHandler" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | head -5
fi
echo

# Check 4: No inline Formatter(...) outside of log_config.py
echo "📋 Check 4: No inline Formatter(...)"
FORMATTER_COUNT=$(grep -RIn "logging\.Formatter" --include="*.py" --exclude="log_config.py" --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | wc -l)
if [ $FORMATTER_COUNT -eq 0 ]; then
  echo "✅ PASS: No inline Formatter(...) usage"
else
  echo "❌ FAIL: Found $FORMATTER_COUNT inline Formatter(...) usage"
  grep -RIn "logging\.Formatter" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | head -5
fi
echo

# Check 5: No calls to logging.basicConfig()
echo "📋 Check 5: No calls to logging.basicConfig()"
BASICCONFIG_COUNT=$(grep -RIn "basicConfig" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | wc -l)
if [ $BASICCONFIG_COUNT -eq 0 ]; then
  ...
  grep -RIn "basicConfig" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | head -5
fi
echo
...
CONFIG_COUNT=$(grep -RIn "logging\.basicConfig\|StreamHandler\|FileHandler" --include="*.py" --exclude="log_config.py" --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | wc -l)
if [ $CONFIG_COUNT -eq 0 ]; then
  ...
  grep -RIn "logging\.basicConfig\|StreamHandler\|FileHandler" --include="*.py" --exclude-dir=log_config.py --exclude-dir=tests --exclude-dir=tools --exclude-dir=sports_venv . | head -5
fi
echo
